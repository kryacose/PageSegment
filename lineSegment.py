...
def lineSegment(img, out, isbin=False, mode = 2, saveRes = True):
    ...
    # Remove lines and find rotation angle
    ...
    heatmap = __heatmap_opt(heatmap)
    heatmap = __heatmap_opt2(heatmap)
    cv2.imwrite(os.path.join(out,'heatmap.jpg'), heatmap)
    global IMG_HM
    IMG_HM = heatmap.copy()
    ...
    # Mark feature on image
    __visualization(bin, heatmap, fps, lpss)

    #Get stright lines of text
    if(mode != 0):
        Res.append(get_sLine(src_warp, heatmap, lpss, saveRes))

    #Get free lines of text
    if(mode != 1):
        ...
        Res.append(get_fLine(ms_blks, saveRes))
